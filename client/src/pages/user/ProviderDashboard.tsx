...
const ProviderDashboard = () => {
  ...
  const recentActivity = [
    { id: 1, type: "booking", service: "Home Cleaning", amount: "$75", date: "2 hours ago" },
    { id: 2, type: "completed", service: "Lawn Mowing", amount: "$45", date: "1 day ago" },
    { id: 3, type: "request", service: "Plumbing Repair", amount: "$120", date: "3 days ago" },
    { id: 4, type: "review", service: "Furniture Assembly", rating: 5, date: "5 days ago" },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{userStats.servicesOffered}</div>
            <p className="text-xs text-muted-foreground mt-1">
              <Link to="/user/my-services" className="text-primary hover:underline">View all</Link>
            </p>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              <Link to="/user/requests" className="text-primary hover:underline">View requests</Link>
            </p>
          </CardContent>
        </Card>
        <Card>
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your latest bookings, requests, and reviews</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {recentActivity.map((activity) => (
                  <div key={activity.id} className="flex justify-between items-center p-3 bg-muted rounded-md">
                    <div>
                      <p className="font-medium">
                        {activity.type === "booking" && "New booking for"}
                        {activity.type === "completed" && "Completed"}
                        {activity.type === "request" && "New request for"}
                        {activity.type === "review" && "Received review for"}{" "}
                        <span className="font-semibold">{activity.service}</span>
                      </p>
        ...
        <div>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Most booked services on the platform</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {popularServices.map((service) => (
                  <div key={service.id} className="flex items-center gap-3">
                    <div
                      className="w-10 h-10 rounded-md flex items-center justify-center bg-muted text-xl"
                    >
                      {service.icon}
                    </div>
